...
#[clap(
    ...
    // `happycode-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `happycode` command name that users run.
    bin_name = "happycode",
    override_usage = "happycode [OPTIONS] [PROMPT]\n       happycode [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run HappyCode non-interactively.
    ...
    /// Manage external MCP servers for HappyCode.
    Mcp(McpCli),

    /// Start HappyCode as an MCP server (stdio).
    McpServer,

    /// [experimental] Run the app server or related tooling.
    AppServer(AppServerCommand),

    /// Launch the HappyCode desktop app (downloads the macOS installer if missing).
    #[cfg(target_os = "macos")]
    ...
    /// Run commands within a HappyCode-provided sandbox.
    Sandbox(SandboxArgs),
    ...
    /// Apply the latest diff produced by HappyCode agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from HappyCode Cloud and apply changes locally.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | happycode login --with-api-key`)"
    )]
    with_api_key: bool,
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating HappyCode via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart HappyCode.");
    Ok(())
}
...
async fn cli_main(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Review(review_args)) => {
            let mut exec_cli = ExecCli::try_parse_from(["happycode", "exec"])?;
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | happycode login --with-api-key`."
                        );
                        std::process::exit(1);
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". HappyCode's interactive TUI may not work in this terminal."
        );
...
mod tests {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run happycode resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run happycode resume my-thread".to_string(),
            ]
        );
    }
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["happycode", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["happycode", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["happycode", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
